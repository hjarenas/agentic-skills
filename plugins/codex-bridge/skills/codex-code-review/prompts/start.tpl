...
If `{{TARGET}}` resolves to a file under `docs/1-plans/`, treat it as the **implementation plan**: read it, evaluate the diff against it. If not a path (e.g. a free-form label), skip "Plan conformance" and review against the architecture patterns in `docs/archi/` plus the stated intent in the additional-context block below.
