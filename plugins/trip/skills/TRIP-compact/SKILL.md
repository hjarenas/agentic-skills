---
name: TRIP-compact
description: "[Legacy] Compact a monolithic ARCHI.md — superseded by /wiki-migrate"
disable-model-invocation: true
---

# ARCHI Compaction Mode (legacy)

> **This skill is superseded.** Compaction buys size by deleting detail, permanently. The
> architecture wiki solves the same problem by *splitting* pages instead of shrinking them, so
> nothing is lost and readers only load the pages they need.
>
> **Prefer `/wiki-migrate`**, which converts `docs/ARCHI.md` into `docs/archi/` once and ends the
> compaction treadmill for good.
>
> Use this skill only when you cannot migrate right now and `ARCHI.md` has become unworkable —
> and treat the result as a stopgap. Tell the user that migrating is the actual fix.
...
```bash
bash TRIP-compact/count-tokens.sh docs/ARCHI.md
```
...
Cross-check with `get_hub_nodes_tool`/`get_bridge_nodes_tool` (code-review-graph): modules that show up as structural hubs or bridges are load-bearing — don't compress their documentation away even if it reads as verbose.
