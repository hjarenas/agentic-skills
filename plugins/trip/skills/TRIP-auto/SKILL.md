...
- The feature branch is always left in a clean, pushed state when stopping mid-way, so work is never lost — with two named exceptions in `TRIP-2-implement` phase scheduling: an unresolved merge conflict deliberately leaves the feature worktree mid-merge (conflict markers present, that phase's work not yet committed or pushed) so a human can resolve or abort it; and a lost merge/cleanup report whose phase-specific recovery audit fails or is ambiguous stops with the merge slot held and the observed repository state untouched. Every other stop condition still leaves the feature branch clean and pushed. Leave the outer feature worktree and any still-open phase worktrees in place rather than removing them, so the user or a resumed session can inspect or continue from exactly where the run stopped.
