...
## Step 0: Read the architecture wiki

Read `docs/archi/index.md`, then the pages bearing on the question. For a question the wiki may
...
Invoke the `codex-ask` skill with a short topic label and your draft position:

```
codex-ask <topic-label> Here is my draft recommendation: <summary + key rationale>.
Red-team it: what am I missing, what would you choose instead, and why?
```

Follow up on the same topic label if the answer raises points worth probing — the skill carries the previous answer into the follow-up. Then:
